...
def extractWordFeatures(x: str) -> FeatureVector:
    ...
    word_features_dict = defaultdict(int)
    word_list = x.split()
    for word in word_list:
        word_features_dict[word] += 1
...
def learnPredictor(trainExamples: List[Tuple[Any, int]], validationExamples: List[Tuple[Any, int]],
    ...
    for epoch in range(numEpochs):
        ...
        for x, y in trainExamples:  # 一句一句loop
            if y < 0:
                y = 0
            else:
                y = 1
            # w = w - alpha*((h-y)phi_x)
            fv_dict = featureExtractor(x)
            h = sigmoid(dotProduct(weights, fv_dict))
            scale = alpha*(-(h-y))
            weights = increment(weights, scale, fv_dict)

        def predictor(check):
            phi_vector = featureExtractor(check)
...
def generateDataset(numExamples: int, weights: WeightVector) -> List[Example]:
    ...
    def generateExample() -> Tuple[Dict[str, int], int]:
        ...
        for i in range(random.randint(1, len(weights))):
            random_key = list(weights.keys())[random.randint(0, (len(weights)-1))]
            phi[random_key] += 1
...
def extractCharacterFeatures(n: int) -> Callable[[str], FeatureVector]:
    ...
    def extract(x: str) -> Dict[str, int]:
        ...
        # raise Exception("Not implemented yet")
        len_phi = defaultdict(int)
        new_x = x.replace(' ', '')
        for i in range(len(new_x)-2):
            len_phi[new_x[i:i+3]] += 1
        return len_phi
        # END_YOUR_CODE

...
def increment(d1, scale, d2):
    ...
    for key in d2:
        d1_value = d1.get(key, 0)
        d1_value += d2.get(key)*scale
        d1[key] = d1_value
    return d1
    # 對應submission.py的weight，對應目標feature vector已進行計算產出(key)的weight(value)，weight可能還不存在對應該feature vector的值
    # END_YOUR_CODE
...
def dotProduct(d1, d2):
    ...
    if len(d1) < len(d2):
        ...
        ans = 0.0
        for key in d2:
            d1_value, d2_value = d1.get(key, 0), d2.get(key, 0)
            ans += d1_value*d2_value
        return ans
        # END_YOUR_CODE
